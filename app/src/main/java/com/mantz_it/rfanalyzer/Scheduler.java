...
public class Scheduler extends Thread {
	...
	public Scheduler(int fftSize, IQSourceInterface source) {
		...
		Log.e(LOGTAG, "Init demodInputQueue size: " + String.valueOf(demodInputQueue.size()));
	}
	...
	public void run() {
		...
		while(!stopRequested) {
			...
			if(demodulationActivated && squelchSatisfied) {
				...
				Log.e(LOGTAG, "demodInputQueue size: " + String.valueOf(demodInputQueue.size()));
				...
				if (demodBuffer != null) {
					...
					Log.e(LOGTAG, "demodOutputQueue size: " + String.valueOf(demodOutputQueue.size()));
				} else {
					...
					/*while ((tmpFlushBuffer = demodOutputQueue.poll()) != null) {
						...
						Log.d(LOGTAG, "insert demod flush");
					}*/
