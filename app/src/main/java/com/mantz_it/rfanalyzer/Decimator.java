...
public class Decimator extends Thread {
	...
	private static final int RETURN_QUEUE_SIZE = 20;		// Double Buffer
	...
	public Decimator (int outputSampleRate, int packetSize, ArrayBlockingQueue<SamplePacket> inputQueue,
		...
		this.inputReturnQueue = new ArrayBlockingQueue<SamplePacket>(RETURN_QUEUE_SIZE);
	...
	public SamplePacket getDecimatedPacket(int timeout) {
		try {
			return inputReturnQueue.poll(timeout, TimeUnit.MILLISECONDS);
